...
"""
        ...
        try:
            ...
            if response and response.choices and len(response.choices) > 0 and response.choices[0].message and response.choices[0].message.content:
                ...
                # --- Пост-обработка: если текст слишком короткий, повторяем запрос ---
                if len(post_text) < int(avg_length * 0.9):
                    logger.warning(f"Сгенерирован слишком короткий пост ({len(post_text)} символов), повторяем запрос с усиленным требованием!")
                    user_prompt += f"\n\nТы выдал слишком короткий текст! Повтори генерацию, но напиши не менее {avg_length} символов, подробно, с деталями, аналитикой, списками, абзацами, пояснениями, выводами и рекомендациями. Не сокращай!"
