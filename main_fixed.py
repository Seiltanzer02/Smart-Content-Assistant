...
# --- НОВЫЕ API ЭНДПОИНТЫ ДЛЯ РАБОТЫ С ПОЛЬЗОВАТЕЛЬСКИМИ НАСТРОЙКАМИ ---
...
async def get_user_settings(request: Request):
    ...
    try:
        ...
        if not telegram_user_id:
            logger.warning("Запрос настроек без идентификации пользователя Telegram")
        ...
        result = supabase.table("user_settings").select("*").eq("user_id", telegram_user_id).execute()
        
        if not hasattr(result, 'data') or len(result.data) == 0:
            # Если настроек нет, возвращаем значения по умолчанию
            logger.info(f"Настройки для пользователя {telegram_user_id} не найдены, возвращаем значения по умолчанию")
            return UserSettings()
        
        # Возвращаем настройки
        settings_data = result.data[0]
        return UserSettings(
            channelName=settings_data.get("channelName"),
            selectedChannels=settings_data.get("selectedChannels", []),
            allChannels=settings_data.get("allChannels", [])
        )
...
async def update_user_settings(request: Request, settings: UserSettings):
    ...
    try:
        ...
        telegram_user_id = request.headers.get("X-Telegram-User-Id")
        if not telegram_user_id:
            logger.warning("Запрос на обновление настроек без идентификации пользователя Telegram")
            raise HTTPException(status_code=401, detail="Для доступа к настройкам необходимо авторизоваться через Telegram")
        ...
        # Проверяем, существуют ли настройки для этого пользователя
        check_result = supabase.table("user_settings").select("id").eq("user_id", telegram_user_id).execute()
        
        settings_data = {
            "user_id": int(telegram_user_id),
            "channelName": settings.channelName,
            "selectedChannels": settings.selectedChannels,
            "allChannels": settings.allChannels,
            "updated_at": datetime.now(timezone.utc).isoformat()
        }
        
        if hasattr(check_result, 'data') and len(check_result.data) > 0:
            # Если настройки существуют, обновляем их
            user_settings_id = check_result.data[0]['id']
            result = supabase.table("user_settings").update(settings_data).eq("id", user_settings_id).execute()
            ...
            logger.info(f"Созданы настройки для пользователя {telegram_user_id}")
        
        if not hasattr(result, 'data') or len(result.data) == 0:
            logger.error(f"Ошибка при сохранении настроек пользователя {telegram_user_id}: Ответ Supabase пуст или не содержит данных.")
