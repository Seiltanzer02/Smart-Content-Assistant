...
@app.post("/check-channel-subscription")
async def check_channel_subscription(request: Request):
    """
    Проверяет, подписан ли пользователь на канал. Если нет — отправляет ему ссылку на канал и инструкцию.
    Ожидает в теле запроса user_id (Telegram ID пользователя).
    """
    data = await request.json()
    user_id = data.get("user_id")
    if not user_id:
