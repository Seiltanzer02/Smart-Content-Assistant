...
Не включай ничего, кроме этих строк."""
        ...
        response = await client.chat.completions.create(
            model="meta-llama/llama-3.2-1b-instruct", # <--- ИЗМЕНЕНО НА НОВУЮ БЕСПЛАТНУЮ МОДЕЛЬ
            messages=[
...
@app.post("/schema_fix")
async def fix_schema():
    """Функция для проверки и исправления схемы базы данных"""
    try:
        # Проверяем наличие заголовка авторизации для операций с базой данных
        # (в реальной среде нужно более серьезное ограничение доступа)
        
        logger.info("Запрос на проверку/обновление схемы базы данных")
        
        if not supabase:
            logger.error("Невозможно выполнить fix_schema: клиент Supabase не инициализирован")
            return {"success": False, "message": "База данных не инициализирована"}
        
        # Добавление столбца is_sample_data в таблицу channel_analysis, если его нет
        try:
            logger.info("Добавление столбца is_sample_data в таблицу channel_analysis")
            result = supabase.rpc(
                'execute_sql', 
                {'sql_query': 'ALTER TABLE channel_analysis ADD COLUMN IF NOT EXISTS is_sample_data BOOLEAN DEFAULT FALSE;'}
            ).execute()
            logger.info(f"Команда выполнена успешно: {result.data[:60] if hasattr(result, 'data') else 'Нет данных'}")
        except Exception as e:
            logger.error(f"Ошибка при добавлении столбца is_sample_data: {e}")
            
        # Добавление столбца used_backup_api в таблицу channel_analysis, если его нет
        try:
            logger.info("Добавление столбца used_backup_api в таблицу channel_analysis")
            result = supabase.rpc(
                'execute_sql', 
                {'sql_query': 'ALTER TABLE channel_analysis ADD COLUMN IF NOT EXISTS used_backup_api BOOLEAN DEFAULT FALSE;'}
            ).execute()
            logger.info(f"Команда выполнена успешно: {result.data[:60] if hasattr(result, 'data') else 'Нет данных'}")
        except Exception as e:
            logger.error(f"Ошибка при добавлении столбца used_backup_api: {e}")
            
        # Создание индекса для analyzed_posts_count в таблице channel_analysis
        try:
            logger.info("Создание индекса для analyzed_posts_count в таблице channel_analysis")
            result = supabase.rpc(
                'execute_sql',
                {'sql_query': 'CREATE INDEX IF NOT EXISTS idx_channel_analysis_analyzed_posts_count ON channel_analysis (analyzed_posts_count);'}
            ).execute()
            logger.info(f"Команда выполнена успешно: {result.data[:60] if hasattr(result, 'data') else 'Нет данных'}")
        except Exception as e:
            logger.error(f"Ошибка при создании индекса для analyzed_posts_count: {e}")
        
        logger.info("Столбцы успешно добавлены или уже существуют")
        
        return {"success": True, "message": "Добавление столбцов выполнено успешно"}
    except Exception as e:
        logger.error(f"Ошибка при выполнении fix_schema: {e}")
        return {"success": False, "message": f"Ошибка при обновлении схемы: {str(e)}"}
