...
async def generate_post_details(request: Request, req: GeneratePostDetailsRequest):
    ...
    try:
        ...
        # Расчет средней длины и добавление искусственного длинного примера, если нужно
...
5. Включать примеры, детали, аналитику, списки, абзацы, пояснения, выводы и рекомендации
6. Не короче {avg_length} символов
...
"""
        ...
        if post_samples:
            sample_text = "\n\n---\n\n".join(post_samples[:5])  # Берем до 5 примеров, чтобы не превышать токены
            user_prompt += f"""
...
Средняя длина постов в этом канале: {avg_length} символов. Новый пост должен быть не короче этой длины!
"""
        user_prompt += f"\n\nПовторяю: Напиши развернутый, подробный, длинный пост не менее {avg_length} символов. Не сокращай!"
        ...
        try:
            ...
            response = await client.chat.completions.create(
                ...
                temperature=0.7,
                max_tokens=1500, # === ИЗМЕНЕНО: Увеличен лимит токенов с 850 до 1500 ===
                timeout=60,
                extra_headers={
                    "HTTP-Referer": "https://content-manager.onrender.com",
                    "X-Title": "Smart Content Assistant"
        ...
        elif len(post_text) < int(avg_length * 0.7):
