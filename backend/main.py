...
async def telegram_webhook(request: Request):
    ...
    if pre_checkout_query:
        ...
        if text.startswith('/start check_premium') or text == '/check_premium':
            ...
            # Проверяем премиум-статус пользователя через REST API
            try:
                # Проверяем, инициализирован ли Supabase клиент
                if not supabase:
                    logger.error("Supabase клиент не инициализирован")
                    await send_telegram_message(user_id, "Ошибка сервера: не удалось подключиться к базе данных.")
...
async def check_db_tables():
    ...
    logger.info("Проверка структуры таблиц в базе данных...")
