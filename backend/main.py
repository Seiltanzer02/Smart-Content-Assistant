...
async def telegram_webhook(request: Request):
    """Обработчик webhook'а от Telegram для обработки событий оплаты."""
    try:
        data = await request.json()
        logger.info(f"Telegram webhook data: {data}")
        
        # проверяем, является ли это успешным обновлением оплаты
        if data.get("update_id") and data.get("message", {}).get("successful_payment"):
            # Получаем данные платежа
            payment = data.get("message", {}).get("successful_payment", {})
            invoice_payload = payment.get("invoice_payload", "")
            
            # Ожидаем, что payload содержит "stars_invoice_{timestamp}_{user_id}"
            if invoice_payload.startswith("stars_invoice_"):
                parts = invoice_payload.split("_")
                if len(parts) >= 4:
                    user_id = parts[-1]  # Последний элемент должен быть user_id
                    
                    # Активируем подписку для пользователя
                    if subscription_service:
                        try:
                            # Используем invoice_payload как payment_id
                            await subscription_service.create_subscription(int(user_id), invoice_payload)
                            logger.info(f"Подписка активирована для пользователя {user_id} через webhook")
                        except Exception as sub_error:
                            logger.error(f"Ошибка при активации подписки через webhook: {sub_error}")
                    ...
                    if supabase:
                        ...
                        try:
                            ...
                            logger.info(f"Информация о платеже сохранена для пользователя {user_id}")
                        except Exception as db_error:
                            logger.error(f"Ошибка при сохранении информации о платеже: {db_error}")
                            
                    return {"success": True, "message": "Payment processed successfully"}

        # Для других типов событий просто возвращаем успешный ответ
...
async def analyze_channel(request: Request, req: AnalyzeRequest):
    ...
    if not telegram_user_id:
        logger.warning("Анализ канала запрошен без идентификации пользователя")
    ...
    # Проверка доступности анализа для пользователя
    if subscription_service:
        try:
            # Проверяем, может ли пользователь выполнить анализ
            ...
            if not can_analyze:
                ...
                if not has_subscription and usage.get("analysis_count", 0) >= FREE_ANALYSIS_LIMIT:
                    logger.warning(f"Пользователь {telegram_user_id} превысил бесплатный лимит анализа ({FREE_ANALYSIS_LIMIT})")
                    raise HTTPException(
                        status_code=402, 
                        detail=f"Вы достигли лимита бесплатных анализов ({FREE_ANALYSIS_LIMIT}). Приобретите подписку Premium для продолжения."
                    )
        except Exception as e:
            logger.error(f"Ошибка при проверке доступности анализа: {e}")
            # Не блокируем доступ в случае ошибки проверки, но логируем ее
    ...
    try:
        ...
        # Увеличиваем счетчик использования анализа, если сервис доступен
        if subscription_service and telegram_user_id:
            try:
                await subscription_service.increment_analysis_usage(int(telegram_user_id))
                logger.info(f"Увеличен счетчик использования анализа для пользователя {telegram_user_id}")
            except Exception as count_error:
                logger.error(f"Ошибка при увеличении счетчика использования анализа: {count_error}")
...
async def generate_post_details(request: Request, req: GeneratePostDetailsRequest):
    ...
    try:
        ...
        # Проверка доступности генерации постов для пользователя
        if subscription_service:
            try:
                # Проверяем, может ли пользователь сгенерировать пост
                can_generate = await subscription_service.can_generate_post(int(telegram_user_id))
                if not can_generate:
                    # Получаем информацию о статусе подписки и использовании
                    has_subscription = await subscription_service.has_active_subscription(int(telegram_user_id))
                    usage = await subscription_service.get_user_usage(int(telegram_user_id))
                    
                    if not has_subscription and usage.get("post_generation_count", 0) >= FREE_POST_LIMIT:
                        logger.warning(f"Пользователь {telegram_user_id} превысил бесплатный лимит генерации постов ({FREE_POST_LIMIT})")
                        raise HTTPException(
                            status_code=402, 
                            detail=f"Вы достигли лимита бесплатных генераций постов ({FREE_POST_LIMIT}). Приобретите подписку Premium для продолжения."
                        )
            except Exception as e:
                logger.error(f"Ошибка при проверке доступности генерации постов: {e}")
                # Не блокируем доступ в случае ошибки проверки, но логируем ее
        ...
        logger.error(f"Ошибка при генерации детального поста: {e}")
        return HTTPException(status_code=500, detail=f"Ошибка при генерации детального поста: {str(e)}")
    
    return PostDetailsResponse(
        generated_text=f"Текст поста: {topic_idea}\nФормат: {format_style}",
        found_images=found_images,
        message=api_error_message,
        channel_name=channel_name
    )

@app.get("/subscription-status", response_model=Dict[str, Any])
async def get_subscription_status(request: Request):
    """Получает статус подписки пользователя."""
    # Получение telegram_user_id из заголовков
    telegram_user_id = request.headers.get("X-Telegram-User-Id")
    if not telegram_user_id:
        logger.warning("Запрос статуса подписки без идентификации пользователя Telegram")
        raise HTTPException(
            status_code=401, 
            detail="Необходима авторизация через Telegram"
        )
    
    # Проверяем, есть ли сервис подписок
    if not subscription_service:
        logger.error("Сервис подписок не инициализирован")
        raise HTTPException(
            status_code=503, 
            detail="Сервис подписок недоступен"
        )
    
    try:
        user_id = int(telegram_user_id)
        
        # Получаем информацию о подписке и использовании
        has_subscription = await subscription_service.has_active_subscription(user_id)
        subscription_data = await subscription_service.get_subscription(user_id)
        usage_data = await subscription_service.get_user_usage(user_id)
        ...
        # Формируем статусы использования
        can_analyze = await subscription_service.can_analyze_channel(user_id)
        can_generate_post = await subscription_service.can_generate_post(user_id)
        
        # Формируем информацию о лимитах
        free_limits = {
            "analysis_limit": FREE_ANALYSIS_LIMIT,
            "post_generation_limit": FREE_POST_LIMIT
        }
        ...
        return {
            ...
            "usage": {
                ...
                "post_generation_count": usage_data.get("post_generation_count", 0),
                "reset_at": usage_data.get("reset_at")
            },
            "can_use": {
                "analysis": can_analyze,
                "post_generation": can_generate_post
            },
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка при получении статуса подписки: {str(e)}"
        )

@app.post("/activate-subscription", response_model=Dict[str, Any])
async def activate_subscription(request: Request):
    """Активирует подписку для пользователя после успешной оплаты."""
    try:
        ...
        if not user_id:
            logger.warning("Попытка активации подписки без указания ID пользователя")
            raise HTTPException(
                status_code=400,
                detail="Необходимо указать ID пользователя"
            )
        
        # Проверяем инициализацию сервиса подписок
        if not subscription_service:
            logger.error("Сервис подписок не инициализирован при попытке активировать подписку")
            raise HTTPException(
                status_code=503,
                detail="Сервис подписок недоступен"
            )
        ...
        # Создаем подписку
        subscription = await subscription_service.create_subscription(int(user_id), payment_id)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка при активации подписки: {str(e)}"
        )
...
async def startup_event():
    ...
    # Проверяем наличие каталога загрузок
    os.makedirs(UPLOADS_DIR, exist_ok=True)
    logger.info(f"Каталог загрузок: {UPLOADS_DIR}")

    # Проверка схемы базы данных и существования таблиц
    try:
        await check_db_tables()
        logger.info("Проверка схемы БД завершена")
    except Exception as e:
        logger.error(f"Ошибка при проверке схемы БД: {e}")
    
    # Инициализация сервиса подписок, если он еще не был инициализирован
    if not subscription_service and supabase:
        try:
            # Импортируем и создаем сервис подписок
            ...
            logger.error(f"Ошибка при инициализации сервиса подписок: {sub_error}")
    
    # Форматирование JSON полей
    try:
        await fix_formatting_in_json_fields()
        logger.info("Форматирование JSON полей завершено")
    except Exception as e:
        logger.error(f"Ошибка при форматировании JSON полей: {e}")
    
    # Исправление форматирования в existing ideas
    try:
        await fix_existing_ideas_formatting()
        logger.info("Исправление форматирования существующих идей завершено")
    except Exception as e:
        logger.error(f"Ошибка при исправлении форматирования существующих идей: {e}")
    
    # Исправление форматирования в существующих постах
    try:
        await fix_existing_posts_formatting()
        logger.info("Исправление форматирования существующих постов завершено")
    except Exception as e:
        logger.error(f"Ошибка при исправлении форматирования существующих постов: {e}")
        
    # Проверка сброса счетчиков использования
    try:
        if subscription_service and supabase:
            # Получаем все записи, где дата сброса прошла или отсутствует
            try:
                result = supabase.table("user_usage_stats").select("user_id").execute()
                if result.data:
                    for user_data in result.data:
                        user_id = user_data.get("user_id")
                        if user_id:
                            # Проверяем счетчики для каждого пользователя
                            await subscription_service.check_reset_counters(user_id)
                    logger.info(f"Проверка сброса счетчиков выполнена для {len(result.data)} пользователей")
            except Exception as reset_error:
                logger.error(f"Ошибка при проверке сброса счетчиков: {reset_error}")
    except Exception as e:
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    # Если это уже HTTPException, просто возвращаем его
    ...
    # Некоторые общие типы исключений преобразуем в соответствующие HTTP-ошибки
    if isinstance(exc, ValueError):
        return JSONResponse(
            status_code=400,
            content={
                "detail": f"Некорректные данные: {str(exc)}",
                "status_code": 400
            }
        )
    ...
    if isinstance(exc, KeyError):
        return JSONResponse(
            status_code=400,
            content={
                "detail": f"Отсутствует обязательное поле: {str(exc)}",
                "status_code": 400
            }
        )
