...
async def openrouter_with_fallback(request_func, *args, **kwargs):
    """Выполняет запрос к OpenRouter с fallback на второй ключ, а затем на OpenAI GPT-3.5 при ошибке."""
    errors = []
    import logging
    logger = logging.getLogger(__name__)
    try:
    ...
    # Сначала пробуем оба OpenRouter ключа
    for api_key in [OPENROUTER_API_KEY, OPENROUTER_API_KEY2]:
        ...
        try:
            ...
            if should_retry:
                ...
                break
    # Если оба OpenRouter не сработали, пробуем OpenAI GPT-3.5
    if OPENAI_API_KEY:
        try:
            logger.info("Попытка вызова OpenAI GPT-3.5 turbo с запасным ключом...")
            client = AsyncOpenAI(api_key=OPENAI_API_KEY)  # base_url по умолчанию для OpenAI
            # Для совместимости: подменяем model на gpt-3.5-turbo, если не передан явно
            def gpt35_request_func(client, *args, **kwargs):
                # args[0] — user_prompt, args[1] — period_days, args[2] — styles, args[3] — channel_name
                # Для генерации плана: только user_prompt и period_days нужны
            ...
            # Если request_func — do_request из generate_plan_llm, то подменяем на gpt35_request_func
            if request_func.__name__ == "do_request":
                return await gpt35_request_func(client, *args, **kwargs)
            else:
                # Для других случаев (например, генерация деталей поста) — аналогично, но с разными параметрами
                return await request_func(client, *args, **kwargs)
        except Exception as e:
            error_message = str(e)
...
# 3. Генерация деталей поста (generate_post_llm)
...
# 4. Генерация ключевых слов для изображений (generate_keywords_llm)
...
# 5. Генерация ключевых слов для поиска изображений (generate_image_keywords)
