...
async def check_user_subscribed_to_channel(user_id: int, channel_username: str) -> Tuple[bool, Optional[str]]:
    """
    ...
    Returns:
        Tuple[bool, Optional[str]]: (подписан ли пользователь, сообщение об ошибке)
    """
    # Удаление символа '@' из имени канала, если он есть
    if channel_username.startswith('@'):
        channel_username = channel_username[1:]
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(telegram_api_url, params={
                "chat_id": f"@{channel_username}",
                "user_id": user_id
            })
            
            if response.status_code != 200:
                logger.error(f"Ошибка API Telegram при проверке подписки: {response.status_code} {response.text}")
                return False, f"Ошибка API Telegram: {response.status_code}"
            ...
            # Статусы участника: creator, administrator, member, restricted, left, kicked
            if status in ["creator", "administrator", "member"]:
                logger.info(f"Пользователь {user_id} подписан на канал @{channel_username}")
                return True, None
            else:
                logger.info(f"Пользователь {user_id} не подписан на канал @{channel_username}, статус: {status}")
                return False, None
                
    except Exception as e:
        logger.error(f"Ошибка при проверке подписки пользователя {user_id} на канал @{channel_username}: {e}")
