...
{sample_text}"""
        ...
        if OPENROUTER_API_KEY:
            try:
                ...
                # --- Анализ стиля и структуры постов ---
                avg_length = 0
                avg_paragraphs = 0
                structural_analysis = ""
        ...
        # Если нет OPENROUTER_API_KEY, но есть OPENAI_API_KEY, используем его напрямую
            ...
            try:
                ...
                # Получаем анализ структуры для случая, когда используется только OpenAI API
                avg_length = 0
                avg_paragraphs = 0
                structural_analysis = ""
                ...
                if post_samples and len(post_samples) > 0:
                    ...
                    openai_tokens = max(100, min(1200, avg_length // 3))
                else:
                    openai_tokens = 850
                
                # Обновляем system_prompt для случая с OpenAI API
                if post_samples:
                    system_prompt = f"""Ты — опытный контент-маркетолог для Telegram-каналов. Твоя задача — сгенерировать текст поста на основе идеи и формата, который будет готов к публикации.
