...
async def generate_post_details(request: Request, req):
    ...
    try:
        ...
        # Усиленный system_prompt с акцентом на копирование стиля только из примеров
        if post_samples:
...
СТРОГО ЗАПРЕЩЕНО использовать любые квадратные скобки [], фигурные скобки {{}}, плейсхолдеры, шаблоны, слова 'ссылка', 'контакт', 'email', 'телефон', 'название', 'дата', 'пример', 'шаблон', 'placeholder', 'link', 'reference', 'details', 'to be filled', 'to be added', 'fill in', 'insert', 'указать', 'оставить', 'заполнить', 'см. ниже', 'см. выше', 'подробнее', 'N/A' и любые другие вставки. В ответе должен быть только готовый текст поста без вставок и шаблонов. Не используй никаких конструкций, требующих заполнения или уточнения. Не используй никаких ссылок, памяток, контактов, email, телефонов, дат, названий, примеров, шаблонов, placeholder'ов, ссылок, reference, details, to be filled, to be added, fill in, insert, указать, оставить, заполнить, см. ниже, см. выше, подробнее, N/A и других подобных вставок. В ответе только готовый текст поста для публикации."""
        if post_samples:
            sample_text = "\n\n---\n\n".join(post_samples[:10])
            user_prompt += f"\n\nВот несколько примеров постов из этого канала для копирования стиля (копируй только их стиль, структуру, форматирование, длину, тональность, особенности подачи, не используй ничего нового):\n{sample_text}\n"
