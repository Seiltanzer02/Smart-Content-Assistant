...
async def generate_post_details(request: Request, req):
    ...
    try:
        ...
        # Анализ тональности из примеров
        tone_instruction = ""
        if post_samples:
...
Создай пост, который логически дополняет развитие канала, но освещает новые аспекты темы."""
        ...
        # Компактные, но важные запреты для готовности к публикации
        user_prompt += """

ВАЖНО для готовности к публикации: Создай законченный контент без шаблонов, плейсхолдеров [], {}, пустых ссылок, мест для заполнения или фраз типа "добавьте свой вариант", "укажите контакты", "см. ниже". Используй конкретные факты, цифры и примеры."""
